...
def worker_main(rank: int, world_size: int, config: DictConfig, policy: nn.Module, reference_model: Optional[nn.Module] = None):
    ...
    # print(f'reference_model is {reference_model}')
    trainer.train()
    ...
    if config.model.archive is None:
        ...
        if config.use_lorafa:
            ...
            peft_config = LoraConfig(
                r=config.lora_rank,
                lora_alpha=config.lora_alpha,
                use_slora=config.use_slora,
                splice_ratio=config.splice_ratio,
                splice_position=config.splice_position,
                lora_dropout=0.05,
                bias="none",
                task_type="CAUSAL_LM",
                target_modules=modules,
        ...
        if config.use_lorafa:
            for name, param in policy.named_parameters():
                if "lora_A" in name:
                    ...
                    print(f'use_lorafa , {name}` grad is  {param.requires_grad}')
        ...
        # SLORA
            for name, param in policy.named_parameters():
                condition = 'lora' in name and (f'.{config.splice_position}.' in name  or f'.{config.splice_position + 1}.'  in name )
                if condition:
        ...
        for name, param in policy.named_parameters():
            if param.requires_grad is True:
                print(f'{name}` grad is  {param.requires_grad}')
...
if __name__ == '__main__':
    ...
    os.environ["WANDB_DISABLED"] = "true"  # wandb
