...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    Args:
        model ([`~transformers.PreTrainedModel`]): The base transformer model used for mora.
        peft_config ([`PeftConfig`]): The configuration of the Peft model.
        adapter_name (`str`,  *optional*): The name of the adapter, defaults to `"default"`.

    **Attributes**:
        - **base_model** ([`torch.nn.Module`]) -- The base transformer model used for mora.
        - **peft_config** ([`PeftConfig`]) -- The configuration of the Peft model.
    ...
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default") -> None:
        super().__init__()
        # print("--"*10,"here was used  PeftModel", "--"*10)
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            save_embedding_layers (`Union[bool, str]`, *optional*, defaults to `"auto"`):
                ...
                embedding layers `mora.utils.other.EMBEDDING_LAYER_NAMES` in config's `target_modules` when available.
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            is_trainable (`bool`, *optional*, defaults to `False`):
                ...
                used for inference.
            config ([`~mora.PeftConfig`], *optional*):
        ...
        for name, module in model.named_modules():
            if any(proj in name for proj in ['k_proj', 'gate_proj', 'v_proj', 'up_proj', 'q_proj', 'o_proj', 'down_proj']):
                module.__name__ = name
                layer_idx = int(name.split('.')[2])
                module.__layer_idx__ = layer_idx
    ...
    def get_prompt(self, batch_size: int, task_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Returns the virtual prompts to use for mora. Only applicable when using a prompt learning method.
        """
    ...
    def set_adapter(self, adapter_name: str) -> None:
        ...
        ```py
        >>> for name, param in model_mora.named_parameters():
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not peft_config.is_prompt_learning:
            ...
            #  modify  add the method of register
            for name, module in self.base_model.named_modules():
                if any(proj in name for proj in
                    ...
                    if hasattr(module, 'lora_A') and hasattr(module, 'lora_B'):
                        matrix_type = name.split('.')[-1]
                        LoraLayer.lora_manager.register_layer(
                            matrix_type,
                            layer_idx,
