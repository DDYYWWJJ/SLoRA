...
class BaseTuner(nn.Module, ABC):
    ...
    The easiest is to check what is done in the `mora.tuners.lora.LoraModel` class.
    ...
    def _prepare_adapter_config(self, peft_config: PeftConfig, model_config: dict) -> PeftConfig:
        ...
        Check out `mora.tuner.lora.LoraModel._prepare_adapter_config` for an example.
    ...
    def _create_and_replace(
        ...
        Check `mora.tuners.lora.LoraModel._create_and_replace` for an example.
    ...
    def _mark_only_adapters_as_trainable(self, model: nn.Module):
        ...
        Check `mora.tuners.lora.LoraModel._mark_only_adapters_as_trainable` for an example.
        """
        ...
    ...
    def inject_adapter(self, model: nn.Module, adapter_name: str):
        ...
        hood by `mora.mapping.get_peft_model` if a non-prompt tuning adapter class is passed.
        ...
        # print(f'key is {key_list}')
        #  这个部分是对各个层进行替换的代码,即q,k,v等模块
        for key in key_list:
            ...
            parent, target, target_name = _get_submodules(model, key)
            # 初始化lora时这里被调用
            # print(f' target_name  is {target_name}')
