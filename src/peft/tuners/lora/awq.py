...
class AwqLoraLinear(torch.nn.Module, LoraLayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rpeft)
    ...
    if is_auto_awq_available() and isinstance(target_base_layer, WQLinear_GEMM):
        ...
        if AUTOAWQ_MINIMUM_VERSION > version_autoawq:
            raise ImportError(
                ...
                f"but only versions above {AUTOAWQ_MINIMUM_VERSION} are supported for mora."
            )
