...
class LoraMatrixManager:
    def __init__(self):
        self._instances = {}

    def get_instance(self, matrix_type: str):
        if matrix_type not in self._instances:
            self._instances[matrix_type] = {
                'A_matrices': {},
                'B_matrices': {},
                'next_index': 0
            }
        return self._instances[matrix_type]

    def register_layer(self, matrix_type: str, layer_idx: int, lora_A: nn.Module, lora_B: nn.Module) -> int:
        instance = self.get_instance(matrix_type)
        if layer_idx not in instance['A_matrices']:
            instance['A_matrices'][layer_idx] = lora_A
            instance['B_matrices'][layer_idx] = lora_B
            instance['next_index'] = max(instance['next_index'], layer_idx + 1)
        return layer_idx
...
class LoraLayer(BaseTunerLayer):
    ...
    lora_manager = LoraMatrixManager()
    ...
    def __init__(self, base_layer: nn.Module, **kwargs) -> None:
        ...
        self.offset = 1
        self.use_slora: dict[str, bool] = {}
        self.splice_ratio = kwargs['splice_ratio']
        self.splice_position = kwargs['splice_position']
        self.start_layer = 0
        self.matrix_type = self._infer_module_type()
        self.layer_idx = self._infer_layer_idx()
    ...
    # modify
    def _infer_module_type(self):
        name = getattr(self.base_layer, "__name__", "")
        if "q_proj" in name:
            return "q_proj"
        elif "query" in name:
            return "query"
            ...
            warnings.warn(f"Unknown module: {name}")
            # print(f'name  is {name}-------')
            return "other"
        #   target_modules=['k_proj', 'gate_proj', 'v_proj', 'up_proj', 'q_proj', 'o_proj', 'down_proj']
    # modify
    def _infer_layer_idx(self):
        name = getattr(self.base_layer, "__name__", "")
        parts = name.split('.')
        ...
        return 0
    def update_layer(
            self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rpeft, use_dora: bool = False,
            use_mora: bool = False, mora_type: int = 1,use_slora: bool = False
    ):
        ...
        self.use_slora[adapter_name] = False
        ...
        if use_mora:
            ...
            # modify
            r = self.r['default']
            self.use_slora[adapter_name] = True
            ...
            self.__class__.lora_manager.register_layer(
                self.matrix_type,
                self.layer_idx,
                self.lora_A[adapter_name],
                self.lora_B[adapter_name]
            )
            manager = self.__class__.lora_manager.get_instance(self.matrix_type)
            A_matrices = manager['A_matrices']
            B_matrices = manager['B_matrices']
            total_layers = len(A_matrices)
            if use_rpeft:
    ...
    def _cache_pop(self, key: str) -> Any:
        ...
        return value
    # modify
    def _apply_slora(self, x, dropout,scaling, active_adapter):
        # modify
        A, B = self._get_combined_matrices(active_adapter)
        x_dropout = dropout(x)
        A = A.to(x_dropout.dtype)
        B = B.to(x_dropout.dtype)
        if x_dropout.dim() == 3:
            original_shape = x_dropout.shape
            x_flat = x_dropout.reshape(-1, original_shape[-1])
            output = x_flat @ A @ B
            output = output.reshape(*original_shape[:-1], output.shape[-1])
        else:
            output = x_dropout @ A @ B
    ...
    def _apply_mora(self, x, lora_A, lora_B, scaling, active_adapter):
        ...
        if mora_type == 1 or mora_type == 4:
            ...
            # print(f'self.mora_type is {self.mora_type} , mora_type is {active_adapter}')
            # print(f"here in_f is {in_f}, out_f is {out_f} r is {r} ,sum_inter is {sum_inter}, rb1 is {rb1}")
            if in_f % r != 0:
                pad_size = r - in_f % r
                x = torch.cat([x, x[..., :pad_size]], dim=-1)
                sum_inter += 1
    ...
    def _apply_dora(self, x, lora_A, lora_B, scaling, active_adapter):
        ...
        if magnitude.dtype != target_dtype:
            magnitude = magnitude.to(target_dtype)
        # print(f'{x.dtype} != {weight.dtype}, magnitude is {target_dtype}')
...
class Linear(nn.Module, LoraLayer):
    ...
    def __init__(
            ...
            use_rpeft: bool = False,
            use_dora: bool = False,
            use_mora: bool = False,
            mora_type: int = 1,
            use_slora: bool = False,
            **kwargs,
    ) -> None:
        import traceback
        # formatted_stack = custom_format_stack(ignore_modules)
        # print('the trace is ', ''.join(formatted_stack))
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.lora_A.keys():
                ...
                if safe_merge:
                    ...
                    if not self.use_dora[active_adapter]:
                        # modify
                        if delta_weight.device != base_layer.weight.data.device:

                            delta_weight = delta_weight.to(base_layer.weight.data.device).detach()
                        # print(delta_weight.device,base_layer.weight.data.device)
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if self.use_mora[adapter]:
            ...
            if adapter not in self.mora_type or self.mora_type[adapter] == 1:
                ...
                i += 1
                w[i * r:, i * r:] = (aw2 * sin[:, i] + aw * cos[:, i])[:,
                                    :r - pad_size]  # + aw2*sin[:, i])[:, :r-pad_size]
                if pad_size > 0:
            ...
            output_tensor = w
        # modify
        elif self.use_slora[adapter]:
            device = self.lora_B[adapter].weight.device
            dtype = self.lora_B[adapter].weight.dtype
            combined_A, combined_B = self._get_combined_matrices(adapter)
            output_tensor = transpose(combined_B.T @ combined_A.T, self.fan_in_fan_out) * self.scaling[adapter]
            # print('xxx'*20)
        else:
    ...
    # modify
    def _get_combined_matrices(self, adapter_name):
        if self.matrix_type is None:
            return self.lora_A[adapter_name].weight.t(), self.lora_B[adapter_name].weight

        manager = self.__class__.lora_manager.get_instance(self.matrix_type)
        A_matrices = manager['A_matrices']
        B_matrices = manager['B_matrices']
        total_layers = len(A_matrices)
        portion = self.splice_ratio
        indices = []
        for i in range(self.offset + 1):
            idx = (self.splice_position + i) % total_layers
            indices.append(idx)
        r = self.r[adapter_name]

        partial_r = int(r * (1 / portion))
        if partial_r < 1:
            partial_r = 1
        ...
        for i in indices:
            ...
            A = A_matrices[i].weight.t()
            B = B_matrices[i].weight
            if i != self.layer_idx:
                combined_A_parts.append(A[:, -partial_r:])
                combined_B_parts.append(B[:, -partial_r:])
            else:
                combined_A_parts.append(A[:, :])
                combined_B_parts.append(B[:, :])
        current_device = next(self.parameters()).device

        def need_device_sync(parts, target_device):
            return any(part.device != target_device for part in parts)

        if need_device_sync(combined_A_parts, current_device):
            combined_A_parts = [part.to(current_device) for part in combined_A_parts]

        if need_device_sync(combined_B_parts, current_device):
            combined_B_parts = [part.to(current_device) for part in combined_B_parts]

        combined_A = torch.cat(combined_A_parts, dim=1)
        combined_B = torch.cat(combined_B_parts, dim=1).t()
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                elif self.use_slora[active_adapter]:
                    # modify
                    result = result + self._apply_slora(x, dropout, scaling, active_adapter)
