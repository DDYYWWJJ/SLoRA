...
def get_peft_model_state_dict(
    ...
    Args:
        ...
        save_embedding_layers (`Union[bool, str]`, , *optional*, defaults to `auto`):
            ...
            layers `mora.utils.other.EMBEDDING_LAYER_NAMES` in config's `target_modules` when available. Based on it
    ...
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA):
        ...
        if bias == "none":
            to_return = {
                k: state_dict[k]
                for k in state_dict
                if "lora_" in k or "combine_scaling_factors" in k
            }
        elif bias == "all":
            to_return = {
                k: state_dict[k]
                for k in state_dict
                if "lora_" in k or "bias" in k or "combine_scaling_factors" in k
            }
        elif bias == "lora_only":
            to_return = {}
            for k in state_dict:
        ...
        to_return = {
            ...
            if (("lora_" in k and adapter_name in k) or
                ("combine_scaling_factors" in k and adapter_name in k) or
                ("splice_ratio" in k and adapter_name in k) or
                ("splice_position" in k and adapter_name in k) or
                ("bias" in k))
        }
...
def set_peft_model_state_dict(model, peft_model_state_dict, adapter_name="default"):
    """
    Set the state dict of the Peft model, including custom scaling factors.
    """
    config = model.peft_config[adapter_name]
    state_dict = {}
    if getattr(model, "modules_to_save", None) is not None:
        ...
        for k, v in state_dict.items():
            if parameter_prefix in k:
                ...
                if "." in suffix:
                    ...
                    mapped_key = k.replace(suffix_to_replace, f"{adapter_name}.{suffix_to_replace}")
                else:
                    mapped_key = f"{k}.{adapter_name}"
                final_state_dict[mapped_key] = v
            else:
                final_state_dict[k] = v
        for k, v in state_dict.items():
            if "combine_scaling_factors_A" in k:

                module_name = k.rsplit(".", 1)[0]
                mapped_key = f"{module_name}.combine_scaling_factors_A.{adapter_name}"
                final_state_dict[mapped_key] = v
            elif "combine_scaling_factors_B" in k:
                module_name = k.rsplit(".", 1)[0]
                mapped_key = f"{module_name}.combine_scaling_factors_B.{adapter_name}"
                final_state_dict[mapped_key] = v
            elif "merge_indices" in k:
                mapped_key = f"{module_name}.merge_indices.{adapter_name}"
                final_state_dict[mapped_key] = v

    elif config.peft_type in (PeftType.LOHA, PeftType.LOKR, PeftType.IA3, PeftType.OFT, PeftType.POLY):
    ...
    if config.is_prompt_learning:
        model.prompt_encoder[adapter_name].embedding.load_state_dict(
            {"weight": final_state_dict["prompt_embeddings"]}, strict=True
        )

    if config.peft_type == PeftType.MULTITASK_PROMPT_TUNING:
        model.prompt_encoder[adapter_name].load_state_dict(final_state_dict, strict=False)
    return load_result


def set_peft_model_state_dict_old(model, peft_model_state_dict, adapter_name="default"):
    """
