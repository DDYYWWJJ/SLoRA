...
def set_gpu_memory_fraction(fraction=0.98):
...
def main():
    ...
    if args.load_generations_path:
        ...
        import yaml    
        model_yaml_path = os.path.join('config', 'model', f"{args.model}.yaml")
        ...
        if args.peft_model:
            ...
            # print(f'args.peft_model is {args.peft_model}')
    ...
    if not args.generation_only:
        if accelerator.is_main_process:
            print(f"Model: {results['config']['peft_model']}")
            ...
            with open(f"{args.metric_output_path}/results/sr_{args.lora_rank}_{args.sparsity_ratio}.txt", "a") as f:
